...
def clean_data(df):
    ...
    categories = categories.set_axis(category_colnames, axis=1, inplace=False)
    
    #Convert category values to just numbers 0 or 1 and clean category values.
    for column in categories:
        # set each value to be the last character of the string
        categories[column] = categories[column].str[-1:]
        # convert column from string to numeric
        categories[column] = pd.to_numeric(categories[column])
        #replace if value '2' exists in column and set to 1. Only 0 and 1 should exist.
        categories[column].replace(2,1,inplace=True) 
        # drop all columns if they contain only one unique value 
        if len(categories[column].unique()) == 1:
            categories.drop([column], axis=1, inplace=True)      
         
    # concatenate the original dataframe with the new `categories` dataframe  
    # and drop original category column
